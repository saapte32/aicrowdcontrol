...
def run(frame):
    ...
    for i in indices:
        ...
        draw_bounding_box(frame, class_ids[i], confidences[i], round(x), round(y), round(w), round(h), centers)
...
def draw_bounding_box(img, class_id, confidence, x, y, w, h, centers):
    if class_id == 0:
        ...
        for i in range(len(centers)):
            for j in range(i + 1, len(centers)):
                if act_dist[i] > 2.00:
                    ...
                    cv2.putText(img, 'Safe', (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                elif act_dist[i] < 1.99:
                    cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 1)
                    cv2.putText(img, 'Unsafe', (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
