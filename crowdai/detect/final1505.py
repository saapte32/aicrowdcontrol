...
def draw_line(image, centers, act_dist):
...
def for_video_out(File):
    cap = cv2.VideoCapture(File)
    number_frame = 30.0
    video_size = (416, 416)
    fourcc = cv2.VideoWriter_fourcc(*'DIVX')
    out = cv2.VideoWriter('Face.mp4', fourcc, number_frame, video_size)  # Saves Video file in mp4 format
    while True:
        ret, frame = cap.read()
        ...
        if ret:
            ...
            out.write(frame)
            image_traffic = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            #         display_img(frame)
...
def for_image_out_sharpened(image):
...
while cap.isOpened():
    ...
    image = draw_line(predict(frame1), centers1, act_dist)
