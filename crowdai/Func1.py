...
# async def getindices(boxes, nms_thresh):
#     x =  [sorted_indices for sorted_indices in np.argsort([-box.classes[c] for box in boxes])
#          for c in range(boxes[0].classes) if len(boxes)>0 ]
...
async def draw_box1(v_boxes, v_labels, v_scores):
    centers = []
    centers1 = []
    for i in range(len(v_boxes)):
        ...
        # label = " %d" % (i + 1)
        # ImageDraw.Draw(image).text((x1, y1), label, (100, 100, 100))
    # frame = np.array(image)
    # frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
    await asyncio.sleep(0.001)
    # future.set_result(frame, centers, centers1)
    return centers, centers1
